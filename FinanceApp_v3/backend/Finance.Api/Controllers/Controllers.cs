...
public class FinanceController : ControllerBase {
  ...
  public async Task<IActionResult> UpSettings([FromBody] UpdateSettingsDto dto) { await _s.UpdateSettings(dto); return Ok(await _s.GetSettings()); }
  ...
  [HttpPost("partners/calculate")] public async Task<IActionResult> Calc([FromQuery] DateTime from, [FromQuery] DateTime to)
  ...
  [HttpGet("dashboard")] public async Task<IActionResult> Dashboard() => Ok(await _s.Dashboard());
  [HttpGet("day-net")] public async Task<IActionResult> DayNet([FromQuery] DateTime date) => Ok(await _s.DayNet(date));

  [HttpGet("incomes")] public async Task<IActionResult> Incomes([FromQuery] DateTime? from, [FromQuery] DateTime? to)
    => Ok(await _s.ListIncomes(from, to));
  [HttpPost("incomes")] public async Task<IActionResult> AddInc([FromBody] AddIncomeDto dto) {
    try { return Ok(await _s.AddIncome(dto, Uid, Uname)); } catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
  }
  [HttpPut("incomes/{id}")][Authorize(Roles = "Owner")] public async Task<IActionResult> EditInc(int id, [FromBody] EditIncomeDto dto) {
    try { await _s.EditIncome(id, dto, Uid, Uname); return Ok(); } catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
  }
  [HttpDelete("incomes/{id}")][Authorize(Roles = "Owner")] public async Task<IActionResult> DelInc(int id, [FromBody] DeleteNoteDto dto) {
    try { await _s.DeleteIncome(id, dto, Uid, Uname); return Ok(); } catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
  }

  [HttpGet("expenses")] public async Task<IActionResult> Expenses([FromQuery] DateTime? from, [FromQuery] DateTime? to)
    => Ok(await _s.ListExpenses(from, to));
  [HttpPost("expenses")] public async Task<IActionResult> AddExp([FromBody] AddExpenseDto dto) {
    try { return Ok(await _s.AddExpense(dto, Uid, Uname)); } catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
  }
  [HttpPut("expenses/{id}")][Authorize(Roles = "Owner")] public async Task<IActionResult> EditExp(int id, [FromBody] EditExpenseDto dto) {
    try { await _s.EditExpense(id, dto, Uid, Uname); return Ok(); } catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
  }
  [HttpDelete("expenses/{id}")][Authorize(Roles = "Owner")] public async Task<IActionResult> DelExp(int id, [FromBody] DeleteNoteDto dto) {
    try { await _s.DeleteExpense(id, dto, Uid, Uname); return Ok(); } catch (InvalidOperationException ex) { return BadRequest(new { message = ex.Message }); }
  }
  ...
  [HttpGet("audits")][Authorize(Roles = "Owner")] public async Task<IActionResult> Audits() => Ok(await _s.Audits());
}

[ApiController][Route("api/[controller]")][Authorize(Roles = "Owner")]
public class UploadsController : ControllerBase {
  private readonly IWebHostEnvironment _env; private readonly FinanceService _s;
  public UploadsController(IWebHostEnvironment env, FinanceService s) { _env = env; _s = s; }
  [HttpPost("{type}")] public async Task<IActionResult> Upload(string type, IFormFile file) {
    ...
    var dir = Path.Combine(_env.ContentRootPath, "wwwroot", "uploads"); Directory.CreateDirectory(dir);
    ...
    var rel = $"/uploads/{name}"; await _s.SetMedia(type, rel); return Ok(new { path = rel });
  }
  [HttpDelete("media/{type}")] public async Task<IActionResult> Clear(string type) {
    if (type is not ("logo" or "loginBg" or "homeBg")) return BadRequest(new { message = "نوع غير صالح" });
    await _s.SetMedia(type, null); return Ok();
  }
}

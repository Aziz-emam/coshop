...
import { Box, Typography, Paper, Button, TextField, Grid, Table, TableHead, TableRow, TableCell, TableBody } from '@mui/material'
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        نشط = يظهر عند صرف الراتب من المصروفات. موقوف = لا يُختار.
      </Typography>
      <Paper sx={{ p: 2, mb: 2 }}>
        <Grid container spacing={1}>
          ...
          <Grid item xs={6} sm={2}><TextField fullWidth size="small" label="الوظيفة" value={form.jobTitle} onChange={e => setForm({ ...form, jobTitle: e.target.value })} /></Grid>
          ...
          <Grid item xs={6} sm={2}><TextField fullWidth size="small" type="date" label="بدء العمل" InputLabelProps={{ shrink: true }} value={form.hireDate} onChange={e => setForm({ ...form, hireDate: e.target.value })} /></Grid>
          <Grid item xs={6} sm={2}><TextField fullWidth size="small" label="ملاحظة" value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} /></Grid>
          <Grid item xs={12} sm={1}>
            <Button fullWidth variant="contained" onClick={async () => {
              try {
                await api.post('/Finance/employees', { ...form, jobTitle: form.jobTitle || null, hireDate: form.hireDate || null, notes: form.notes || null })
      ...
      <Table size="small" component={Paper}>
        <TableHead>
          <TableRow>
            <TableCell>الاسم</TableCell><TableCell>وظيفة</TableCell><TableCell>راتب</TableCell>
            <TableCell>بدء العمل</TableCell><TableCell>الحالة</TableCell>
              ...
              {user?.role === 'Owner' && (
                <TableCell>
                  ...
                  <Button size="small" color="error" onClick={async () => {
                    ...
                    await api.delete(`/Finance/employees/${e.id}`); toast.success('تم'); load()
                  }}>حذف</Button>
                </TableCell>
              )}
            </TableRow>

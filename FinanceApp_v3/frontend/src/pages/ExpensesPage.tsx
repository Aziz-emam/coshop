...
export default function ExpensesPage() {
  ...
  const loadMeta = async () => {
    ...
    } catch { toast.error('تعذر تحميل البنود') }
    try {
  ...
  const load = async (p = page) => {
    try {
      ...
      setPage(a.data.page || 1)
      setTotalPages(a.data.totalPages || 1)
      setTotalCount(a.data.totalCount || 0)
    } catch {
          ...
          <Grid item xs={12} sm={2}>
            <Button fullWidth variant="contained" color="secondary" onClick={async () => {
              ...
              try {
                ...
                toast.success('تم')
                setForm({ operationDate: today(), categoryId: 0, amount: 0, notes: '', partnerId: '', employeeId: '' })
                load(1)
              ...
              {user?.role === 'Owner' && (
                <TableCell>
                  ...
                  <Button size="small" color="error" onClick={async () => {
                    ...
                    if (!note?.trim()) return
                    try {
                      await api.delete(`/Finance/expenses/${x.id}`, { data: { editNote: note } })
                      toast.success('تم')
                      load(page)
                    } catch (e: any) { toast.error(e?.response?.data?.message || 'فشل') }
                  }}>حذف</Button>
        ...
        <DialogContent>
          {edit && (
            <Grid container spacing={1} sx={{ mt: 0.5 }}>
              <Grid item xs={6}>
                <TextField fullWidth size="small" type="date" label="تاريخ" InputLabelProps={{ shrink: true }}
                  value={edit.operationDate} onChange={e => setEdit({ ...edit, operationDate: e.target.value })} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth size="small" select label="البند" value={edit.categoryId}
              ...
              {selEdit?.code === 'SALARY' && (
                <Grid item xs={6}>
                  <TextField fullWidth size="small" select label="الموظف" value={edit.employeeId || ''}
              ...
              <Grid item xs={12}>
                <TextField fullWidth size="small" required label="ملاحظة التعديل"
          ...
          <Button variant="contained" onClick={async () => {
            ...
            try {
              await api.put(`/Finance/expenses/${edit.id}`, {
                operationDate: edit.operationDate,
                categoryId: edit.categoryId,
                amount: edit.amount,
                notes: edit.notes,
                partnerId: edit.partnerId ? +edit.partnerId : null,
                employeeId: edit.employeeId ? +edit.employeeId : null,
                editNote
              })
              toast.success('تم')
              setEdit(null)
